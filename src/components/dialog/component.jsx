...
const ProjectUpdateDialog = ({ trigger, target }) => {
    ...
    const memberParser = (list) => {
        let temp = [];
        list.forEach((member) => {
            temp.push(`${member.name}/${member.role}`);
        });
        return temp.join(", ");
    };
    const linkParser = (list) => {
        let temp = [];
        list.forEach((link) => {
            temp.push(`${link.type}/${link.url}`);
        });
        return temp.join(", ");
    };
    useEffect(() => {
        setMember(memberParser(target.member));
        setLink(linkParser(target.link));
    }, []);
    return (
        <div
            className="dialogWrap"
